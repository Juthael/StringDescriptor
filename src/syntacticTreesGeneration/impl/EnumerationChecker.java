...
public class EnumerationChecker implements IEnumerationChecker {
	...
	public EnumerationChecker(boolean wholeStringIsDescribed, String dimension, List<String> values) 
	...
	public IEnumerationRelationalData getEnumerationRelationalData() throws SynTreeGenerationException {
		if (simpleEnumerationWasFound == true || secondDegreeEnumerationWasFound == true) {
			IEnumerationRelationalData enumerationRelationalData = 
					new EnumerationRelationalData(dimension, enumerationValue);
		...
		if (values.size() > 1) {
			...
			while (valueIndex < values.size() && allValuesAreSequences == true) {
				...
				if (listOfSubValues.size() != 1) {
					...
					IEnumerationChecker enumerationChecker = new EnumerationChecker(false, "temporaryDimension", listOfSubValues);
