...
public class RelationDataContainerBuilder implements IRelationDataContainerBuilder {
	...
	public RelationDataContainerBuilder(ISignal signal, List<Frame> listOfDescriptors) {
		this.signal = signal;
	...
	public IRelationDataContainer getRelationDataContainer() throws SynTreeGenerationException {
		...
		if (listOfDescriptors.size() > 1) {
			...
			boolean wholeStringIsDescribed = 
					DescriptorSpanGetter.testIfWholeStringIsDescribed(signal, listOfAbstractDescriptors);
					...
					switch (pathLastWord) {
						...
						case "enumeration" :
							...
							IEnumerationChecker enumerationChecker = 
									new EnumerationChecker(wholeStringIsDescribed, dimension, listsOfValues);
							...
							if (simpleEnumerationWasFound == true) {
								...
								ISequenceChecker sequenceChecker = 
										new SequenceChecker(wholeStringIsDescribed, dimension, listsOfValues, 
								...
								ISymmetryChecker symmetryChecker = 
										new SymmetryChecker(wholeStringIsDescribed, dimension, listsOfValues, 
												enumerationRelationalData);
